...
### <a name="prepare-an-rhel-7-virtual-machine-from-hyper-v-manager"></a>Hyper-V Yöneticisi'nden bir RHEL 7 sanal makinesini hazırlama
...
1. Azure Stack için özel bir vhd oluştururken 2.2.20 2.2.35 (her iki özel) arasındaki WALinuxAgent sürümü, Azure Stack ortamlarında çalışmıyor aklınızda bulundurun. Görüntünüzü hazırlamak için sürümleri 2.2.20/2.2.35 sürümleri kullanabilir. Özel görüntünüzü hazırlamak için sürümleri 2.2.35 yukarıda kullanmak için Azure Stack 1903 sürümüne güncelleştirin veya 1901/1902 düzeltmeyi uygulayın. 

     WALinuxAgent indirmek için aşağıdaki yönergeleri izleyin:
   ...
   a.   Setuptools indirin
    ...
    cd setuptools-7.0
    ```
   b. Bir örnek burada biz "2.2.20" indirme budur GitHub deposundan sürümü. İndirip sıkıştırmasını 2.2.20 GitHub'ımızı aracı sürümü. 
    ```bash
    ...
    ```
    e. Aracı sürümü bir, indirilen eşleşiyorsa, test edin. Bu örnekte, 2.2.20 olmalıdır.
...
### <a name="prepare-an-rhel-7-virtual-machine-from-vmware"></a>Vmware'den bir RHEL 7 sanal makinesini hazırlama
...
Farklı sanallaştırma sistemi (diğer bir deyişle, Oracle VM VirtualBox, Xen proje, vb.), Linux görüntüsünü hazırlamak için kullanırken, en az emin olmak için initrd yeniden oluşturmanız gerekebilir hv_vmbus ve hv_storvsc çekirdek modülleri başlangıç RAM kullanılabilir disk. Bilinen bir sorun en az Yukarı Akış Red Hat dağıtım tabanlı sistemlerde budur.
