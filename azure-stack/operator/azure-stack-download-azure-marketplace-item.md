...
- **Bağlantısı kesilmiş veya kısmen bağlı bir senaryo** -Market öğelerini indirme için Market Dağıtım Aracı'nı kullanarak İnternet'e erişmesi gerekir. Ardından, bağlantısı kesilmiş Azure Stack yüklemenizi indirmelerinizi aktarın. Bu senaryo, PowerShell kullanır.
...
Azure Stack dağıtımınıza internet bağlantınız ve olması gerekir [Azure Active Directory'ye](azure-stack-registration.md ).
...
3. Azure Stack Marketini açın ve Azure'a bağlanın. Bunu yapmak için **Market öğesi**ve ardından **Ekle azure'dan**:

    ![Azure'dan ekleyin](media/azure-stack-download-azure-marketplace-item/marketplace.png)

    Portal, Azure Market'ten indirilebilir öğelerin listesini görüntüler. Ürün adı, yayımcı ve/veya ürün türüne göre filtre uygulayabilirsiniz. Ayrıca, açıklaması ve indirme boyutu gibi ek bilgileri görüntülemek için her bir öğede tıklayabilirsiniz:
...
### <a name="prerequisites"></a>Önkoşullar
- Azure Stack dağıtımınıza olmalıdır [Azure Active Directory'ye](azure-stack-registration.md ).  
...
5. Dağıtım modülü içeri aktarın ve ardından aşağıdaki komutları çalıştırarak Aracı'nı başlatın. Değiştirin `Destination folder path` Azure Marketi'nden de dosyaları depolamak için bir konum.   
   ...
   Unutmayın `Export-AzSOfflineMarketplaceItem` ek bir sahip `-cloud` bulut ortamı belirten bayrak. Varsayılan olarak, olduğu **azurecloud**.
