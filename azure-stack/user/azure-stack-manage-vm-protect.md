...
description: Veri kaybı ve planlanmamış kesinti süreleri karşı Azure Stack üzerinde dağıtılan sanal makineleri korumak için bir kurtarma planı oluşturmayı öğrenin.
...
ms.locfileid: "67492420"
---
# <a name="protect-vms-deployed-on-azure-stack"></a>Azure Stack üzerinde dağıtılan sanal makinelerini koruma
...
Kurtarma planına Azure Stack altyapı hizmetleri için **yok** kullanıcı Vm'lerinin, depolama hesapları veya veritabanları kurtarma sayılabilir. Uygulama sahibi olarak, uygulamalar ve veriler için bir kurtarma planı uygulamak için sorumlu.
...
* Kullanıcı isteklerine hizmet tutmak uygulamaları sağlar.

Azure Stack bulut operatörü, temel alınan Azure Stack altyapı ve Hizmetleri için bir kurtarma planı oluşturmak için sorumludur. Daha fazla bilgi için bkz. [geri dönülemez veri kaybından kurtarma](../operator/azure-stack-backup-recover-data.md).
...
Her Azure Stack Bulutu, bir veri merkezine dağıtılır. Uygulamalarınızı kurtarmak için ayrı bir ortam gerekir. Kurtarma Ortamı'nı, başka bir Azure Stack Bulutu farklı bir veri merkezinde veya Azure genel bulutunda olabilir. Uygulamanız için Kurtarma Ortamı'nda veri hakimiyeti ve veri gizlilik gereksinimleri belirleyin. Her uygulama için korumayı etkinleştirin gibi her biri için belirli bir kurtarma seçeneği esnekliğine sahip olursunuz. Başka bir veri merkezi için veri yedekleyen bir Abonelikteki uygulamalara sahip olabilir. Başka bir abonelikte, Azure ortak bulutuna veri çoğaltma yapabilirsiniz.

Her uygulama için hedef belirlemek her uygulama için yedekleme kurtarma ve olağanüstü durum kurtarma stratejinizi planlayın. Bir kurtarma planı, kuruluşunuzun düzgün şekilde kapasite gerekli şirket içi depolama boyutu ve genel bulut tüketimi proje yardımcı olur.
...
| **Müşteri veri merkezine dağıtılan ve müşteri tarafından işletilen azure Stack** | Kullanıcı VM'ler için müşteri tarafından işletilen Azure Stack dağıtılır.<br><br>Kullanıcı Vm'lerinin yedeklemeden geri veya doğrudan Azure'a yük devretti. | Müşteri, Azure Stack birincil sitede çalışır.<br><br>CSP'ın datacenter geri yükleme ya da yük devretme hedefidir. | Müşteri, Azure Stack birincil ve ikincil örneklerini, kendi veri merkezlerinde çalışır.<br><br>Kullanıcı Vm'leri geri veya iki Azure Stack örnekleri arasında yük devretti. |

![Kaynak-hedef birleşimleri](media/azure-stack-manage-vm-backup/vm_backupdataflow_01.png)

## <a name="app-recovery-objectives"></a>Uygulama Kurtarma hedefleri

Her uygulama için kuruluşunuzun tolere edebilen kapalı kalma süresi ve veri kaybı miktarını belirler. Niceleme kapalı kalma süresi ve veri kaybı, kuruluşunuzdaki bir olağanüstü durum etkisini en aza indiren bir kurtarma planı oluşturabilirsiniz. Her uygulama için göz önünde bulundurun:

 - **Kurtarma süresi hedefi (RTO)**  
RTO, bir uygulama bir olay sonrasında kullanılamıyor olmasının kabul edilebilen maksimum süre olan. Örneğin, 90 dakikada bir RTO uygulamayı çalışır bir duruma 90 dakika içinde bir olağanüstü durum başlangıcından geri yükleme olanağınız olmalıdır anlamına gelir. Düşük bir RTO varsa, bölgesel bir kesintiye karşı korumak için bekleme sürekli çalıştıran ikinci bir dağıtım tutabilirsiniz.
 - **Kurtarma noktası hedefi (RPO)**  
RPO, olağanüstü bir durum sırasında kabul edilebilir veri kaybı maksimum süresidir. Örneğin, saatlik yedekleme desteklenir ve başka bir veritabanına çoğaltma içeren tek bir veritabanında veri depolama, veri bir saate kadar kaybına neden olabilir.

RTO ve RPO iş gereksinimleridir. Uygulamanın RTO ve RPO tanımlamak için bir risk değerlendirmesi gerçekleştirin.

Başka bir ölçüm olan **ortalama süresi, kurtarılır** (MTTR) olan bir hatadan sonra uygulamayı geri yükleme için geçen ortalama süre. MTTR, bir sistem için Deneysel bir değerdir. MTTR RTO'yu aşıyorsa, sistemin tanımlanmış RTO içerisinde geri yüklenmesi mümkün olmayacağından sonra bir hata halinde sistemin kabul edilemez iş kesintileri neden olur.

### <a name="backup-restore"></a>Yedekleme geri yükleme

Sanal makine tabanlı uygulamalar için en yaygın koruma şeması, yedekleme yazılımı kullanmaktır. Genellikle bir VM'yi yedekleme, işletim sistemi, işletim sistemi yapılandırması, uygulama ikili dosyalarını ve uygulama verilerini içerir. Yedeklemeler, birimlerin, diskler veya tüm VM anlık görüntüsünü alarak oluşturulur. Azure Stack ile gelen konuk işletim sistemi veya Azure Stack depolama bağlam içinde yedekleme esnekliğine sahip olursunuz ve API işlem. Azure Stack, hiper yönetici düzeyinde yedeklemeleri almak desteklemiyor.
...
Azure Stack üzerinde sanal makineleri yedeklemeye önemli noktalar:
 ...
 - **Geri yükleme**
    - Yedekleme ürünleri, sanal diskler, varolan bir VM'yi veya tüm VM kaynak uygulama verilerini ve ilişkili sanal diskler geri yükleyebilirsiniz. Gereksinim duyduğunuz geri yükleme düzeni, uygulamayı geri yükleme planınızı nasıl üzerinde bağlıdır. Örneğin, SQL Server'dan bir şablonu yeniden dağıtın ve ardından VM kümesi ve tüm VM'yi geri yüklemek yerine veritabanlarını geri yüklemek daha kolay olabilir.

### <a name="replicationmanual-failover"></a>Çoğaltma/el ile yük devretme

Yüksek kullanılabilirliği desteklemek için alternatif bir yaklaşım, uygulamanızı Vm'leri, başka bir buluta çoğaltın ve el ile yük devretme sırasında kullanan oluşturmaktır. İşletim sistemi, uygulama ikili dosyalarını ve uygulama verilerini, çoğaltma, VM düzeyinde veya konuk işletim sistemi düzeyinde yapılabilir. Yük devretme, uygulamanın bir parçası olmayan ek yazılımları kullanarak yönetilir.

Bu yaklaşım, uygulamanın bir bulutta dağıtılır ve kendi VM diğer buluta çoğaltılır. Bir yük devretme tetikleniyorsa, ikincil VM'ler ikinci buluta açık gerekir. Bazı senaryolarda yük devretme Vm'leri ve ekler diskleri bunlara oluşturur. Bu işlem gerektiren belirli başlatma sırası özellikle bir çok katmanlı uygulama ile tamamlanması uzun sürebilir. Uygulama istekleri hizmet vermeye başlaması hazır hale gelmeden önce çalıştırılması gereken adımları da olabilir.
...
Burada, iş yalnızca birkaç saniye veya dakika kapalı kalma süresini minimum düzeyde veri kaybı ve tolere edebilen uygulamaları için yüksek kullanılabilirlik yapılandırmasında göz önünde bulundurun. Yüksek kullanılabilirlik uygulamaları hızla ve otomatik olarak hataları kurtarmak için tasarlanmıştır. Yerel donanım hatalarına için Azure Stack altyapısının fiziksel ağ raf üstü anahtar iki üst kullanarak yüksek kullanılabilirlik uygular. İşlem düzeyi hataları için Azure Stack bir ölçek birimi birden çok düğüm kullanır. VM düzeyinde düğüm hataları uygulamanızı yakalayana emin olmak için hata etki alanları ile ölçek kümeleri ile birlikte kullanabilirsiniz.
...
Bazı uygulamalar, ortamınızdaki planlanmamış kesinti veya veri kaybına karşı koruma gerekmeyebilir. Örneğin, VM'ler, geliştirme için kullanılan ve test genellikle gerekmeyen kurtarılması. Bu uygulama veya belirli bir sanal makine için koruma olmadan yapmak için kararıdır. Azure Stack yedekleme veya çoğaltma sanal makinelerinin temel altyapısından sunmaz. Benzer şekilde, Azure, her bir aboneliğinizde her VM için koruma katılımı gerekir.
...
| Çoğaltma/yük devretme VM'ler için ayrı bir Azure Stack örneği | Önerilen | Yük devretme durumunda, genişletilmiş uygulama kapalı kalma süresini önlemek ikinci bir Azure Stack bulut tam olarak işlevsel olması gerekir. |
...
| Azure Stack çözüm yüklendiği aynı raf fiziksel yedekleme uygulamasına dağıtma | Desteklenmiyor | Şu anda, herhangi bir cihaza özgün çözümün bir parçası olmayan raf üstü anahtarları üstüne bağlanamıyor. |
